            ...
            {projects.length > 0 && (
                <>
                    ...
                    >
                        ...
                        />
                        <span>
                            搜索到 <b style={{ color: '#1677ff' }}>{filteredProjects.length}</b> 个项目
                        </span>
                        <span>
                            已选 <b style={{ color: '#52c41a' }}>{selectedRowKeys.length}</b> / <b>{projects.length}</b> 个项目
                        </span>
                    </div>
                        ...
                        <Table
                            rowSelection={{
                                ...
                                onSelectAll: (selected: any) => {
                                    if (selected) {
                                        // 全选所有项目
                                        filteredProjects.forEach(p => p.selected = true)
                                        setSelectedRowKeys(filteredProjects.map(p => p.name));
                                    } else {
                                        // 取消全选
                                        filteredProjects.forEach(p => p.selected = false)
                                        setSelectedRowKeys([]);
                                    }
                                },
                            }}
